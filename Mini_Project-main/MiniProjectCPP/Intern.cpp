...
void intern::percentile_generator(list<Employee>& thelist)
{
    ...
    fstream fout;
    fout.open("SystemGenerated-DATA.txt", ios::out );
...
{ int i,flag=0;
	...
	fstream f2out;
        f2out.open(nname, ios::out );
        f2out<<"NAME:"<<iter->name<<"\n"<<"ID:"<<iter->id<<"\n"<<"Date Of Birth:"<<iter->dob<<"\n"<<"STATE:"<<iter->state<<"\n"<<"CPP Programming:"<<iter->cpp<<"\n"<<"PYTHON:"<<iter->python<<"\n"<<"LINUX:"<<iter->linuxs<<"\n"<<"SDLC:"<<iter->sdlc<<"\n\n\n\n";
f2out<<"-----------------------------------------------------------------------------------------------------------------------------\n";

        f2out<<"|";
        for(i=0;i<=5*(iter->cpp)/10;i++)
            f2out<<">>";
        f2out<<"("<<iter->cpp<<")";
        f2out<<"Cpp\n";

        f2out<<"|";
        for(i=0;i<=5*(iter->python)/10;i++)
            f2out<<">>";
        f2out<<"("<<iter->python<<")";
        f2out<<"Python\n";

        f2out<<"|";
        for(i=0;i<=5*(iter->sdlc)/10;i++)
            f2out<<">>";
        f2out<<"("<<iter->sdlc<<")";
        f2out<<"SDLC\n";

        f2out<<"|";
        for(i=0;i<=5*(iter->linuxs)/10;i++)
            f2out<<">>";
        f2out<<"("<<iter->linuxs<<")";
        f2out<<"LINUX\n";
f2out<<"-----------------------------------------------------------------------------------------------------------------------------\n\n\n\n";
	...
	cout<<"\n\n\n\n\n==> ==>**"<<iid<<"'s Portfolio has been saved under filename : "<<iter->name<<".txt\n\n\n";
